...
public:
	...
	bool parse_resquest(const char *);
	bool packet_resquest(std::string  *);
	...
	bool get_response_StatusCode(std::string * stdstr);
	...
	bool parse_response(const char *);
	bool packet_response(std::string *);
	struct response mresponse_struct;


private:
	bool deal_request_url(std::string &);
	bool deal_cookie(std::string &str, int);
	enum name { DEALREQUSET, DEALRESPONSE };
};
