...
//const char* http::HTTP_VERSION            = "HTTP-Version";
//
//const char* http::REQUEST_LINE            = "Request-Line";
//const char* http::METHOD                  = "Method";
//const char* http::REQUEST_URI             = "Request-URI";
//
//const char* http::STATUS_LINE             = "Status-Line";
//const char* http::STATUS_CODE             = "Status-Code";
//const char* http::REASON_PHRASE           = "Reason-Phrase";
...
bool http::get_server_addr_from_url( const std::string& url, std::string& server_host, uint16& server_port )
{
	...
	bool isequal;
	#if (defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__NT__))
			isequal = _strnicmp(url_body.c_str(), SCHEME, scheme_size) == 0;
	#elif defined(__linux__) || defined(__linux)
			isequal = strncasecmp(url_body.c_str(), SCHEME, scheme_size) == 0;
	#endif

    if ( url_body.size() > scheme_size &&  isequal)
    {
...
bool http::decode_request( std::istream& is, REQUEST& request, bool with_body )
{
    ...
    {
         //is.seekg (0, is.end);
        ko_int32_t  curr = is.tellg();
        is.seekg (0, is.end);
        ko_int32_t endd=is.tellg();
        content_length = endd - curr;
...
{        
    ...
    if ( ! request.request_line.body_param.empty() )
    {
        str += "?";
        std::map<std::string, std::string>::iterator fbody = request.request_line.body_param.begin(),
        ebody = request.request_line.body_param.end() ; 

        for (; fbody !=ebody ; ++fbody)
        {
            if (iac > 0)
            {
                str += "&" ;
    ...
    if ( ! request.cookiekv.empty() )
    {
         str += "Cookie: ";
         std::map<std::string, std::string>::iterator fcookie = request.cookiekv.begin(),
        ecookie  = request.cookiekv.end();
        iac=0;
        for (; fcookie !=ecookie ; ++fcookie)
        {
            if (iac > 0)
            {
                str += "; " ;
